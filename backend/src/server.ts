...
fastify.post('/api/messages', async (request, reply) => {
  ...
  try {
    const { receiverId, content, type, data } = request.body as {
      receiverId: string
      content: string
      type?: string
      data?: any
...
fastify.post('/api/invitations/game', async (request, reply) => {
  ...
  try {
    const { receiverId, gameType } = request.body as {
      receiverId: string
      gameType: 'pong' | 'tank'
    }
...
fastify.post('/api/invitations/game/:id/accept', async (request, reply) => {
  ...
  try {
    ...
    const invitationMessage = messages.find(msg => 
      msg.type === 'game_invitation' && 
      msg.data && 
...
function sendTournamentSystemMessage(tournament: Tournament, message: string) {
